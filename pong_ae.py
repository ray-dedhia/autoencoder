"""
Autoencoder that runs on images from OpenAI Gym Pong-v0 game.
"""
...
class train_autoencoder:
	...
	def autoencoder(self, input_layer, encodings):
		...
		for i in range(N):
			self.conv_i = self.conv_layer(input_layer, encodings[i][0], i)
		...
		for i,units in enumerate([256,64,256,dim_prod]):
			self.dense_i = tf.layers.dense(inputs=input_layer, units=units, activation=tf.nn.relu, name="dense{}".format(i))	
			input_layer = self.dense_i
		...
		for i in range(N):
			...
			self.trans_conv_i = self.trans_conv_layer(self.unpool_i, deconv_info[i], i)
